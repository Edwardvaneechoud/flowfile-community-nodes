...
`python -m flowfile_core.flowfile.community_nodes.cli` — a module that ships in a Flowfile
release. The gating release is **`flowfile 0.13.1`**, the first PyPI build to ship
`community_nodes`. The launch switches below are applied in this repo; **push them only
once `flowfile==0.13.1` is live on PyPI**, or CI can't install the validator.

- [x] `registry/config.json` `validator_flowfile_version` and `min_supported_app_version`
      pinned to `0.13.1`.
- [x] Both workflows (`validate-pr.yml` / `build-index.yml`) install the validator from
      PyPI (`pip install "flowfile==$(jq -r .validator_flowfile_version registry/config.json)"`);
      the `TEST_FLOWFILE_SPEC` feature-branch install is removed.
